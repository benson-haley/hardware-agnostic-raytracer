...
public:
	...
	Material(
		...
		const ShaderType& shadow_shader = [](const ObjectType&, const MaterialInfo&) { return Vector3{ 0, 0, 0 }; }
	) :
		shader{ shader },
...
inline Vector3 phong(
	...
	Real n_dot_l = std::max(info.normal.dot(l), 0.0);
	...
	Real r_dot_v = std::max(r.dot(-info.position.normalized()), 0.0);
	// Calculate the specular color.
	Vector3 resultant_specular_color = specular_color.cwiseProduct(info.light_color) * std::pow(r_dot_v, shininess);
